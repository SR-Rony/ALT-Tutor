...
import { useTeacherKeyConcepts, useTeacherSubjectsTree } from "@/hooks";
...
export function TeacherKeyConceptsPage() {
  ...
  const { data, isLoading, error, refetch, isFetching } = useTeacherKeyConcepts(
  ...
  if (treeLoading && programs.length === 0) {
    return (
      <div className="space-y-6">
        <PageHeader
          title="Key Concepts"
          description="Read-only view of lessons for your assigned programs."
          className="mb-0"
        />
    ...
    <div className="overflow-hidden rounded-2xl border border-border bg-card shadow-[0_8px_30px_rgba(15,23,42,0.04)]">
      <div className="border-b border-border px-5 py-6">
        <div className="mb-4 flex flex-wrap items-start justify-between gap-3">
          <PageHeader
            title="Key Concepts"
            description="Lessons for programs you’re assigned to (or linked via your courses). Create and publish stay with Admin."
            className="mb-0"
          />
          <AdminIconAction
            label="Refresh"
        ...
        {selected ? (
          <p className="mt-3 text-xs text-muted-foreground">
            Student hub:{" "}
            <Link
              href={ROUTES.subjectResource(selected.slug, "key-concepts")}
              className="inline-flex items-center gap-1 font-semibold text-primary hover:underline"
              target="_blank"
            >
              /subjects/{selected.slug}/key-concepts
      ...
      <div className="space-y-3 p-5">
        {!effectiveProgramId ? null : isLoading ? (
          <PageLoader label="Loading lessons..." className="min-h-[160px]" />
        ) : lessons.length === 0 ? (
          <p className="py-8 text-center text-sm text-muted-foreground">
            No key concept lessons for this program yet.
          </p>
        ) : (
          lessons.map((item) => (
            <div
            ...
            >
              <div className="min-w-0">
                <div className="flex flex-wrap items-center gap-2">
                  ...
                  <span className="rounded-md bg-primary-muted px-1.5 py-0.5 text-[10px] font-bold uppercase text-primary">
                    {contentLabel(String(item.contentType))}
                  </span>
              ...
              {selected && item.isPublished ? (
                <Link
                  href={ROUTES.subjectKeyConceptLesson(selected.slug, item.slug)}
